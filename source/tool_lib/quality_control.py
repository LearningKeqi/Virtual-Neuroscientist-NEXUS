...
#     Returns:
#         A dictionary with:
#           - "directory": the input directory path
#           - "count": number of entries
#           - "entries": list of {"name": str, "entry_type": "file" | "directory", "size_bytes": int | None}
#     """
#     if not os.path.isabs(directory_path):
#         raise ValueError(f"`directory_path` must be an absolute path. Got: {directory_path!r}")
...
@tool
def run_python_script(command: str, job_name: str, need_gpu: bool = False):
    """
    Run a python script.

    Args:
    - command: the command to run the python script. E.g., `python path/to/script.py`.
    - job_name: the name of the job, can only include special character '_'.
    - need_gpu: whether to use GPU.

    Returns:
    - The final log content of the job.
    """
    ...
    # Workspace where scripts and logs are stored (must match the rest of the system)
    workspace_dir = "Path/temp_workspace"
    os.makedirs(workspace_dir, exist_ok=True)

    # Sanitize job name for filesystem and SLURM
    safe_job_name = "".join(
        c if c.isalnum() or c in ("-", "_") else "_"
        for c in (job_name or "job")
    ) or "job"

    # Use .sh extension for the sbatch script as required
    sbatch_script_path = os.path.join(workspace_dir, f"{safe_job_name}.sh")
    log_path = os.path.join(workspace_dir, f"{safe_job_name}_log.txt")
    ...
    header_lines = [
        ...
        f"#SBATCH --gres=gpu:{1 if need_gpu else 0}",
        "#SBATCH --cpus-per-task=8",
        "#SBATCH --mem=64GB",
    ]

    script_content = "\n".join(header_lines + ["", command, ""])  # final newline

    # Write SBATCH script to workspace
    ...
    try:
        # Submit job via sbatch and block until it finishes.
        # This is simpler and relies on SLURM's own waiting mechanism.
        submit_result = subprocess.run(
            ["sbatch", "--wait", sbatch_script_path],
            capture_output=True,
            text=True,
            env=env,
        )

        # # If submission or execution failed at the SLURM level, report it immediately.
        # if submit_result.returncode != 0:
        #     truncated_stdout, _ = truncate_head_tail(submit_result.stdout)
        #     truncated_stderr, _ = truncate_head_tail(submit_result.stderr)
        #     return (
        #         f"SBATCH submission/execution failed (return code {submit_result.returncode}).\n\n"
        #         f"STDOUT:\n{truncated_stdout}\n\n"
        #         f"STDERR:\n{truncated_stderr}\n\n"
        #         f"SBATCH script path: {sbatch_script_path}\n"
        #         f"Expected log path: {log_path}\n"
        #     )

        # At this point, sbatch --wait has returned; the job should be finished.
        # We still wait a short while for the log file to appear and be flushed.
        max_wait_seconds = 300  # up to 5 minutes for log file to materialize
        ...
        # Give SLURM a moment to flush the final log content
        time.sleep(2)
        ...
        #     truncated_stderr, _ = truncate_head_tail(submit_result.stderr)
        #     return (
        #         "Job finished (sbatch --wait returned), but log file was not found.\n\n"
        #         f"sbatch STDOUT (possibly helpful):\n{truncated_stdout}\n\n"
        #         f"sbatch STDERR (possibly helpful):\n{truncated_stderr}\n\n"
        #         f"Expected log path: {log_path}\n"
        #         f"SBATCH script path: {sbatch_script_path}\n"
        #     )
        ...
        truncated_log, is_truncated = truncate_head_tail(log_content)

        header = [
            f"Full Log path: {log_path}",
        ]
        header_text = "\n".join(header) + "\n\n"

        if is_truncated:
            body = f"Log content (truncated: head and tail):\n{truncated_log}\n"
        else:
            body = f"Log content:\n{truncated_log}\n"
